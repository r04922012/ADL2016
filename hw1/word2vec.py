...
class Word2Vec(object):
  ...
  def build_graph(self):
    ...
    self._loss = loss
    self.optimize(loss)
    self.norm_emb = tf.nn.l2_normalize(self._emb, 1)
...
def main(_):
  ...
  with tf.Graph().as_default(), tf.Session() as session:
    ...
    # Perform a final save.
      output_embedding = model.norm_emb.eval()
    ...
    with open('tmp_dir/tmp_word2vec.txt','w') as output:
      for voc_index in xrange(opts.vocab_size):
        output.write(voc_list[voc_index])
        for embed_dimension in xrange(output_embedding.shape[1]):
          output.write(' {}'.format(output_embedding[voc_index][embed_dimension]))
        output.write('\n')
